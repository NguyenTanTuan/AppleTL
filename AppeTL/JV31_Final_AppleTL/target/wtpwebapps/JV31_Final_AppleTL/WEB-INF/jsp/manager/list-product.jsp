							...
							<div class="row">
								<div class="col-md-6">
									<a style="font-size: 25px;"
										href="<c:url value="/manager/product"/>">
										 Product With Discount </a>
								</div>
								<div class="col-md-6" style="text-align: right;">
									<a style="font-size: 20px;"
										href="<c:url value="/manager/downloadCSVproduct"/>">
										Export Product </a>
								</div>
							</div>
								...
								<c:forEach items="${product}" var="pr">
									<tr>
										...
										<%-- 	<c:if test="${products1!=null || fn:length(products1)>0}">
											<c:forEach items="${products1 }" var="pr1">
												<c:if test="${pr.id == pr1.id && pr.price != pr1.price}">
													<td>${pr1.price}</td>
												</c:if>
											</c:forEach>
										</c:if>
